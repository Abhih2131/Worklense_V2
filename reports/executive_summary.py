...
def prepare_performance_distribution_data(df):
    if 'performance_rating' not in df.columns:
        return pd.DataFrame({"performance_rating":[]})
...
def render_line_chart(df, x, y, template):
    if df.empty or x not in df.columns or y not in df.columns:
        st.warning(f"No data for {y} line chart.")
        return
...
def render_bar_chart(df, x, y, template):
    if df.empty or x not in df.columns or y not in df.columns:
        st.warning(f"No data for {y} bar chart.")
        return
...
def render_pie_chart(df, names, values, template):
    if df.empty or names not in df.columns or values not in df.columns:
        st.warning(f"No data for {names} pie chart.")
        return
...
def render_donut_chart(df, names, values, template):
    if df.empty or names not in df.columns or values not in df.columns:
        st.warning(f"No data for {names} donut chart.")
        return
...
def render_bell_curve(df, col, template):
    ...
    if df.empty or col not in df.columns:
        st.warning(f"No data for {col} bell curve.")
        return
    data = df[col].dropna()
    if data.empty:
...
def run_report(data, config):
    ...
    headcount_start = ((filtered_df['date_of_joining'] <= fy_start) & ((filtered_df['date_of_exit'].isna()) | (filtered_df['date_of_exit'] > fy_start))).sum()
    headcount_end = ((filtered_df['date_of_joining'] <= fy_end) & ((filtered_df['date_of_exit'].isna()) | (filtered_df['date_of_exit'] > fy_end))).sum()
    avg_headcount = (headcount_start + headcount_end) / 2 if (headcount_start + headcount_end) else 1
    attrition = (leavers / avg_headcount) * 100 if avg_headcount else 0
